...
class UserAccountServiceTest {
    ...
    void givenExistentUserId_whenSearching_thenReturnsOptionalUserData() {
        ...
        given(userAccountRepository.findById(username)).willReturn(Optional.of(createUserAccount("uno")));

        // When
        Optional<UserAccountDto> result = sut.searchUser(username);

        // Then
        assertThat(result).isPresent();     // 옵셔널 result 값이 null이 아니어야 하고
        then(userAccountRepository).should().findById(username);    // 내부에서 userAccountRepository.findById(...)가 호출되어야 한다.
    }
    ...
    void given_when_then() {
        ...
        // Mock 객체인 userAccountRepository에서 "wrong-user"로 findById를 하면 empty 옵셔널 객체를 반환하도록 지시
        ...
        // Then
        assertThat(result).isEmpty();   // given에서 설정한 값에 따라 empty 객체가 반환되어야 한다.
